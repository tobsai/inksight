"""Stroke smoothing algorithms for InkSight Cloud.
...
Copied from device/inksight/smoothing.py with minimal modifications.
"""
...
from .presets import ProcessingPreset
...
def process_stroke(points: List[Point], preset: ProcessingPreset) -> List[Point]:
    """Apply all configured processing to a single stroke based on preset."""
    result = list(points)

    # Apply smoothing
    if preset.smoothing_enabled and len(result) >= 5:
        result = smooth_gaussian(result, window_size=5, sigma=preset.gaussian_sigma)

    # Apply RDP simplification
    if preset.rdp_enabled and preset.rdp_epsilon > 0:
        result = simplify_rdp(result, epsilon=preset.rdp_epsilon)

    # Apply line straightening
    if preset.line_straightening_enabled:
        result = straighten_line(
            result,
            threshold=preset.straightness_threshold,
            min_length=preset.min_line_length,
            max_points=30,
        )

    # Apply pressure normalization
    if preset.pressure_normalization_enabled:
        result = normalize_pressure(result, target_min=10, target_max=245)
